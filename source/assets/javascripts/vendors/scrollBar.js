  ...
  $.extend(Plugin.prototype, {
    ...
    addScrollbar: function() {
      ...
      this.scollbarContainer = $(
        "<div class='" + this.settings.scrollbarContainerClass + "' />"
      );
      this.scrollBar = $(
        "<div class='" + this.settings.scrollBarClass + "' />"
      );
    ...
    addEvents: function() {
      ...
      this.scrollBar.on(
        "mousedown." + pluginName,
        $.proxy(this.onMousedown, this)
      );
      this.scrollBar.on(
        "touchstart." + pluginName,
    ...
    onTouchstart: function(ev) {
      ...
      var onMove = function(end) {
        ...
        me.scrollBar[0].style.top =
          Math.min(
            me.scollbarContainer[0].clientHeight - me.scrollBar[0].clientHeight,
            Math.max(0, y + delta)
          ) + "px";
        me.wrapper[0].scrollTop =
          (me.wrapper[0].scrollHeight * me.scrollBar[0].offsetTop) /
          me.scollbarContainer[0].clientHeight;
      };
    ...
    onResize: function() {
      ...
      this.scrollBar.css(
        "height",
        (this.scollbarContainer[0].clientHeight * wrapper_client_height) /
          this.wrapper[0].scrollHeight +
          "px"
      );
      if (
        this.scollbarContainer[0].clientHeight <= this.scrollBar[0].clientHeight
      ) {
    ...
    onScroll: function() {
      this.scrollBar.css(
        "top",
        Math.min(
          this.scollbarContainer[0].clientHeight -
            this.scrollBar[0].clientHeight,
          (this.scollbarContainer[0].clientHeight * this.wrapper[0].scrollTop) /
            this.wrapper[0].scrollHeight
        ) + "px"
      );
    }
